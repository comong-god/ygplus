...
const getDefaultLinkActions = (connected: boolean) => {
  return [
    <Link to={`/`} key={'explore'}>
      <Button className="app-btn">메인화면</Button>
    </Link>,
    <Link to={`/artworks`} key={'artwork'}>
      <Button className="app-btn">{connected ? '전체상품' : '전체상품'}</Button>
    </Link>,
    <Link to={`/artists`} key={'artists'}>
      <Button className="app-btn">보유상품</Button>
    </Link>,
  ];
};
...
export const MetaplexMenu = () => {
  ...
  if (width < 768)
    return (
      <>
        <Modal
          title={<img src={'https://clas.ygplus.com/images/logo_ti.png'} />}
...
export const LogoLink = () => {
  return (
    <Link to={`/`}>
      <img src={'https://clas.ygplus.com/images/logo_ti.png'} />
    </Link>
  );
};
